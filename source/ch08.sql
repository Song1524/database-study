...
-- 오류 발생: WHERE 절은 그룹화가 이루어지기 전, 테이블의 개별 행 하나하나에 대해 조건을 검사하기 때문
GROUP BY ptype
...
-- 사용자 주문 총액 필터링하기
-- 사용자별 주문 총액을 구하고, 그 총액이 30000원 이상인 주문자의 주문자명과 주문 총액?
SELECT u.nickname, SUM(amount) AS `주문 총액`
FROM orders o
...
-- 가장 많이 팔린 상품 TOP3
-- 상품별 팬매 수량을 집계했을 때, 가장 많이 팔린 상품 TOP3의 상품명과 판매 수량은?
SELECT p.name AS `가장 많이 팔린 상품`, SUM(od.count) AS `판매 수량` -- 6
FROM order_details od -- 1
JOIN products p ON od.product_id = p.id --2
JOIN orders o ON od.order_id = o.id -- 3
WHERE o.status = '배송 완료' -- 4
GROUP BY od.product_id -- 5
...
SELECT 
	name AS '상품명',
	SUM(p.price * count) AS '누적 매출'
FROM products p
JOIN order_details od ON p.id = od.product_id
JOIN orders o ON od.order_id = o.id
	AND status = '배송 완료'
GROUP BY name
ORDER BY SUM(p.price * count) DESC
LIMIT 3;
